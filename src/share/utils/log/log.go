...
func NewLogger(srvName string) *zap.Logger {
	dir := path.Join(config.LogPath, srvName)
...
func newRollingFile(directory string) zapcore.WriteSyncer {
	if err := os.MkdirAll(directory, 07666); err != nil {
		log.Println("fail create log directory", directory, ":", err)
		return nil
	}
...
type lumberjackWriteSynccer struct {
...
func newLumberjackWriteSyncer(l *lumberjack.Logger) *lumberjackWriteSynccer {
	ws := &lumberjackWriteSynccer{
		Logger:    l,
...
func (l *lumberjackWriteSynccer) run() {
...
func (l *lumberjackWriteSynccer) Stop() {
	close(l.closeChan)
}

func (l *lumberjackWriteSynccer) Write(bs []byte) (int, error) {
	b := make([]byte, len(bs))
	for i, c := range bs {
		b[i] = c
	}
...
func (l *lumberjackWriteSynccer) sync() error {
	defer l.buf.Reset()
